...
// Use the direct PostgreSQL client for all database operations
...
export class AuthService {
  ...
  static async authenticateUser(email: string, password: string): Promise<LoginResult> {
    ...
    try {
      ...
      // Find user by email
      const userQuery = `
        SELECT id, email, password_hash, name, company, avatar_url as avatar, role, subscription, active, last_login_at
        FROM users 
