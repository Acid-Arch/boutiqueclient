...
interface CapacityCheckResponse {
	totalCapacity: {
		...
		brokenClones: number;
		utilizationRate: number; // Percentage of clones in use
	};
	deviceCapacities: Array<{
		deviceId: string;
	...
	assignmentRecommendations: {
		recommendedStrategy: 'round-robin' | 'fill-first' | 'capacity-based';
		reasoning: string;
		canAssignAccounts: number; // How many more accounts can be assigned
		bottleneckDevice?: {
			deviceId: string;
			issue: string;
		};
...
interface DeviceFilterOptions {
	deviceId?: string;
	deviceIds?: string[];
	minAvailableClones?: number;
	includeUnhealthy?: boolean;
	status?: 'all' | 'available' | 'assigned' | 'logged-in' | 'broken';
}

// Cache for capacity data to improve performance
let capacityCache: {
	data: CapacityCheckResponse;
	timestamp: number;
	ttlMs: number;
} | null = null;

const CACHE_TTL_MS = 30000; // 30 seconds cache
const PERFORMANCE_WARNING_THRESHOLD_MS = 5000; // Warn if queries take longer than 5 seconds

// Background cache warming (optional optimization)
let cacheWarmingInterval: NodeJS.Timeout | null = null;

// Auto-warm cache every 25 seconds to ensure fresh data
function startCacheWarming() {
	...
	cacheWarmingInterval = setInterval(async () => {
		try {
			...
			console.error('[CacheWarming] Failed to refresh cache:', error);
		}
	}, 25000); // Refresh every 25 seconds (before 30s TTL expires)
}

// Start cache warming on module load
startCacheWarming();

// GET /api/devices/capacity-check - Real-time capacity checking
export const GET: RequestHandler = async ({ url }) => {
	...
	try {
		...
		const deviceIds = deviceIdsParam ? deviceIdsParam.split(',') : undefined;
		...
		const status = url.searchParams.get('status') as 'all' | 'available' | 'assigned' | 'logged-in' | 'broken' || 'all';
		const useCache = url.searchParams.get('cache') !== 'false'; // Default to true
		...
		const filters: DeviceFilterOptions = {
			deviceId,
			deviceIds,
			minAvailableClones: minAvailableClones ? parseInt(minAvailableClones) : undefined,
			includeUnhealthy,
			status
		};
		...
		if (useCache && isCacheableRequest && capacityCache) {
			...
			if (age < capacityCache.ttlMs) {
				...
				console.log(`[${requestId}] Cache hit - age: ${age}ms, response time: ${responseTime}ms`);
				
				// Update real-time metrics but keep cached data
...
// POST /api/devices/capacity-check - Check capacity for specific assignment scenario
export const POST: RequestHandler = async ({ request }) => {
	...
	try {
		...
		if (accountIds.length > 0 && accountIds.some((id: any) => typeof id !== 'number' || id <= 0)) {
			console.log(`[${requestId}] Invalid accountIds provided`);
			throw error(400, 'accountIds must be an array of positive integers');
		}

		// Get base capacity data
		const filters: DeviceFilterOptions = {
			deviceIds: preferredDevices.length > 0 ? preferredDevices : undefined,
			includeUnhealthy: false,
			status: 'available'
		};
		...
		// Calculate if assignment is possible
		...
		// Simulate assignment to show which devices would be used
		let assignmentPlan: Array<{
			deviceId: string;
			clonesToAssign: number;
...
// Core function to fetch capacity data
async function getCapacityData(filters: DeviceFilterOptions, requestId?: string): Promise<Omit<CapacityCheckResponse, 'realTimeMetrics'>> {
	const logPrefix = requestId ? `[${requestId}]` : '[CapacityData]';
	const prisma = await getPrisma();
	
	// Build query for clones
	let cloneWhereClause: any = {};
	...
	// Process data by device (optimized for performance)
	...
	const deviceMap = new Map<string, {
		...
		lastScanned: Date;
		deviceHealth: string | null;
		statusCounts: { available: number; assigned: number; loggedIn: number; broken: number; };
	}>();

	// Pre-calculate status counts during iteration to avoid multiple passes
	...
	// Calculate device capacities (using pre-calculated counts for performance)
	const deviceCapacities = Array.from(deviceMap.entries()).map(([deviceId, data]) => {
		...
		// Apply minimum available clones filter
	...
	totalCapacity.utilizationRate = totalCapacity.totalClones > 0 ? 
		Math.round(((totalCapacity.assignedClones + totalCapacity.loggedInClones) / totalCapacity.totalClones) * 10000) / 100 : 0;
...
function generateAssignmentRecommendations(
	deviceCapacities: any[],
	totalCapacity: any
): CapacityCheckResponse['assignmentRecommendations'] {
	const availableDevices = deviceCapacities.filter(d => d.availableClones > 0);
	
	// Determine recommended strategy
	let recommendedStrategy: 'round-robin' | 'fill-first' | 'capacity-based' = 'round-robin';
	let reasoning = '';

	// Find bottleneck device (if any)
	let bottleneckDevice: { deviceId: string; issue: string } | undefined;
	...
	if (availableDevices.length === 0) {
		...
		if (capacityVariance > 50) { // High variance in device capacities
		...
		} else if (avgUtilization > 70) { // High overall utilization
			recommendedStrategy = 'round-robin';
...
// Calculate capacity variance helper
function calculateCapacityVariance(devices: any[]): number {
...
// Simulate assignment for POST endpoint
function simulateAssignment(
	devices: any[],
	requiredSlots: number,
	strategy: 'round-robin' | 'fill-first' | 'capacity-based'
): Array<{deviceId: string; clonesToAssign: number; remainingCapacity: number}> {
	
	const plan: Array<{deviceId: string; clonesToAssign: number; remainingCapacity: number}> = [];
	...
	if (strategy === 'round-robin') {
		...
		// Fill-first or capacity-based (sequential filling)
		for (const device of sortedDevices) {
			if (remainingSlots <= 0) break;
...
// Get suggested alternatives when assignment fails
async function getSuggestedAlternatives(
	capacityData: any,
	requiredSlots: number
): Promise<Array<{strategy: string; description: string; feasible: boolean}>> {
	const alternatives = [
		{
			...
			feasible: capacityData.totalCapacity.assignedClones > 0 // Some might become available
		},
		{
